...
check_if_within <- function(sections, entirely = TRUE) {

    range1 <- sections[1]:sections[2]
    range2 <- sections[3]:sections[4]

    fnc <- if(entirely) all else any
    overlap1 <- fnc(range1 %in% range2)
    overlap2 <- fnc(range2 %in% range1)

    check <- (overlap1 | overlap2)

    return(check)
}

solve <- function(input, entirely = TRUE) {
    input |>
        parse_input() |>
        purrr::map(check_if_within, entirely = entirely) |>
        unlist() |>
        sum()
}
