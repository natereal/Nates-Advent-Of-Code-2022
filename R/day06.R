...
check_last_n_unique <- function(input, index, n = 4) {
    if(index < n) return(FALSE)
    num_unique <- input[(index-n+1):index] |>
        unique() |>
        length()
    if(num_unique != n) {
        return(FALSE)
...
find_uniques <- function(input, n = 4) {
    output <- c()
    for(i in 1:length(input)) {
        output[i] <- check_last_n_unique(input, i, n = n)
    }
    return(output)
}

solve <- function(input, n = 4, sum_fnc = min) {
    input |>
        process_input() |>
        find_uniques(n = n) |>
        which() |>
...
example_data_06 <- function(example = 1) {
  l <- list(
    a = c("mjqjpqmgbljsphdztnvjfqwrcgsmlb"),
    b = c("bvwbjplbgvbhsrlpgdmjqwftvncz"),
    c = c("nppdvjthqldpwncqszvftbrmjlhg"),
    d = c("nznrnfrfntjfmvfwmzdfjlvtqnbhcprsg"),
    e = c("zcfzfwzzqfrljwzlrfnpqdbhtmscgvjw")
  )
  l[[example]]
}
